...
files = {
    k: {
        sample: glob.glob(
            os.path.join(output_dir, _.format(sample=sample)), recursive=True
        )
        for _ in v
...
```{python}
def get_barcounter_metrics(logfile: os.PathLike):
...
def get_chromap_metrics(logfile: os.PathLike, summary: os.PathLike):
    with open(logfile, mode="r") as file:
...
def get_macs2_metrics(peakfile: os.PathLike):
...
**Library type** assay for transposase-accessible chromatin  
**Total Reads** total number of sequenced read pairs  
...
if any(itertools.chain.from_iterable(files["chromap"].values())) and any(
    ...
    df_chromap = pd.concat(df_chromap).sort_values(by="Sample").reset_index(drop=True)
    df_macs2 = [
        get_macs2_metrics(files[0])
        .apply(lambda x: pd.to_numeric(x, downcast="integer", errors="coerce"))
        .dropna(axis=1)
        .assign(Sample=sample)
        for sample, files in files["macs2"].items()
    ]
    df_macs2 = pd.concat(df_macs2).sort_values(by="Sample").reset_index(drop=True)
...
**Library type** antibody-derived tags  
**Total Reads** total number of sequenced read pairs  
**% Valid Barcode** fraction of reads with cell barcode matching whitelist  
...
if any(itertools.chain.from_iterable(files["barcounter"].values())):
    df = [
        get_barcounter_metrics(files[0])
        .apply(lambda x: pd.to_numeric(x, downcast="integer", errors="coerce"))
        .dropna(axis=1)
        .assign(Sample=sample)
